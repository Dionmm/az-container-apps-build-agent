...
if [ -z "${AZP_TOKEN_FILE}" ]; then
  if [ -z "${AZP_TOKEN}" ]; then
    if [ -z "${CLIENT_ID}" ]; then
      echo 1>&2 "error: missing CLIENT_ID environment variable and no AZP_TOKEN variable provided"
      exit 1
    fi
    azure_devops_application_id="499b84ac-1321-427f-aa17-267ca6975798"
    response=$(curl -s -X GET -H "X-IDENTITY-HEADER: $IDENTITY_HEADER" "$IDENTITY_ENDPOINT?resource=$azure_devops_application_id&client_id=$CLIENT_ID&api-version=2019-08-01")
